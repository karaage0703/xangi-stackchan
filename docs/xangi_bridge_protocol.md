...
| コマンド | 引数 | 応答 | 状態 |
|---------|------|------|------|
| `STATUS` | なし | `{"state","volume","version","servo","torque","queued","playing"}` JSON | WAV シリアル受信実装 で初版、サーボ統合 PR で `servo`/`torque`、WAV キュー化 PR で `queued`/`playing` 追加 |
